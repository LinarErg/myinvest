...
Процедура ПолучитьИЗаписатьЦенуАктиваНаДату(Актив,Период) Экспорт
	...
	ИначеЕсли Актив.Вид = Перечисления.ВидАктива.Облигация тогда
		ТорговаяСистема = Справочники.СписокТорговыхСистем.НайтиПоНаименованию("Фондовый рынок и рынок депозитов").Заголовок;
		Рынок 			= Справочники.Рынок.НайтиПоНаименованию("Рынок облигаций").Заголовок;
		Если Лев(Актив.Тикер,2) = "SU" тогда
			РежимТоргов = Справочники.РежимТоргов.НайтиПоНаименованию("Т+: Гособлигации - безадрес.").Заголовок;	
		Иначе
			РежимТоргов = Справочники.РежимТоргов.НайтиПоНаименованию("Т+: Облигации - безадрес.").Заголовок;	
		КонецЕсли;
	ИначеЕсли Актив.Вид = Перечисления.ВидАктива.ETF тогда
		ТорговаяСистема = Справочники.СписокТорговыхСистем.НайтиПоНаименованию("Фондовый рынок и рынок депозитов").Заголовок;
		Рынок 			= Справочники.Рынок.НайтиПоНаименованию("Рынок акций").Заголовок;
		РежимТоргов 	= Справочники.РежимТоргов.НайтиПоНаименованию("Т+: ETF - безадрес.").Заголовок;		
	Иначе
		Возврат;
	КонецЕсли;
